...
\begin{document}
    ...
    Which means when ever a client demands a geofencing operation, they send a http query. In this style, fences are very dynamic. The fence will be checked against the whole data base of mover locations. \textbf{location-aggregate} is responsible for this style of fencing. Since sending query to a system is like polling data out of it, the parts of FenceX making this possible are considered poll leg. So \textbf{location-aggregate} is poll leg of FenceX.
    ...
    design and architectueral decisions.
    ...
    \begin{figure}[ht]
        \caption{FenceX infrastructure setup for evaluation - larger size available in the appendix}
        \label{fig:infrastructure}
        \includegraphics[scale=images/Infrsutracture.png]
        \centering
    \end{figure}
    ...
    \textit{fence_event_log, location-aggregate-mover-in-memory-state-store-changelog, mover-position-updates,
    ...
    \paragraph{}
    bench-marking can send HTTP requests to realtime-fencin and define fences for movers.
    ...
    \begin{center}
        \begin{tabular}{ c c c c}
            Application               &     #of instances        & CPU(MHz)  & RAM(MB)    \\
            location-update-publisher &          4               & 500       &   500      \\
            location-aggregate        &          3               & 2000 Mhz  &   2500 MB  \\
            realtime-fencing          &                          & 700       &   800      \\
        \end{tabular}
    \end{center}

    \paragraph{Results}
    \begin{figure}[ht]
        \caption{FenceX push throughput ex1-1 - larger size available in the appendix}
        \label{fig:ex1-1}
        \includegraphics[scale=images/evaluation/ex1-benchmarking(15,6).png]
        \centering
    \end{figure}

    \begin{figure}[ht]
        \caption{FenceX push throughput ex1-2 - larger size available in the appendix}
        \label{fig:ex1-2}
        \includegraphics[scale=images/evaluation/ex1-benchmarking(19,7).png]
        \centering
    \end{figure}

    \begin{figure}[ht]
        \caption{FenceX push throughput ex1-3 - larger size available in the appendix}
        \label{fig:ex1-3}
        \includegraphics[scale=images/evaluation/ex1-benchmarking(22,9).png]
        \centering
    \end{figure}

    \begin{figure}[ht]
        \caption{FenceX push throughput ex1-4 - larger size available in the appendix}
        \label{fig:ex1-4}
        \includegraphics[scale=images/evaluation/ex1-benchmarking(23,10).png]
        \centering
    \end{figure}
