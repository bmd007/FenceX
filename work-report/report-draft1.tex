...
\begin{document}
    ...
    In FenceX, for poll leg, similar to previous experiments, load is rate of incoming HTTP request for queries by
    fence.
    And, for push leg is the rate at which location reports arrive.

    Performance in case of our experiments for FenceX specifically is push leg and poll leg throughput.
    ...
    Figures in following push leg string scalability experiments can also be read in the same way as experiment 8.
    ...
    \end{figure}
    As we expected, deploying three intances of realtime-fencing with the given resources, was enough to process 15K
