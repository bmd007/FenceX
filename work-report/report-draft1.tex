...
\begin{document}
    ...
    Fundamentally each of those state stores are backed by a topic (chagelog).
    ...
    Location-aggregate is a Kafka Streams applications.
    ...
    deployed tightly togheger with location-aggregate.
    ...
    Usually database systems are configured to use disk as storage which allows them grow very big.
    ...
    image of the world.
    H2 provides an out-of-the-box solution for keeping its inatances consistent, even when configured to be embedded
    and in memory.
    ...
    which means they get events from all of the partitions.
