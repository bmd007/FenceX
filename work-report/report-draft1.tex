...
\begin{document}
    ...
    functions.
    In this thesis, thanks to usage of Kafka Stream as the stream processing framework and its supports for stateful
    operations, FenceX allows for definition and deployment of stateful functions.
    ...
    safe zone.
    And then ask the system to notify them by sending a notification to their phone when ever the cat goes out side
    of that fence.
    ...
    These are some of common issues with monolith\cite{monolith} applications.
    ...
    The easiest approach at the moment to write a code and deploy it in cloud is to write it as a function;
    then gave the code to one of the FaaS (function as a service) providers like AWS lambda\cite{lambda}.
    They will do the all required work in order to expose that function, as an HTTP api for example, to internet.
    ...
    office, microservices are packaged as docker\cite{pro-docker} images and deployed into a container orchestration
    tool provided as a managed cloud service.
    ...
    The row in these tables with no values corresponds to a microservice which does not relate to the experiments 8 to 11.
    The following figures illustrate how input rate and throughput have changed during those experiments.
    ...
    During experiments 8 to 11, we have sent an ongoing stream of location updates to FenceX with fixed rate of 15K
    ...
    resources, can only handle one 3rd of input rate.
    ...
    since throughput can not exceed input rate regardless of processing power.

    So as a result of experiments 8 to 11, we can claim that push leg of FenceX has strong scalability characteristics
    ...
    The row in these tables with no values corresponds to a microservice which does not relate to the experiments 17 and 18.
    The following figures illustrate how input rate and throughput have changed during experiments 17 and 18.
    ...
    This is due to the fact that pull leg has a global in memory data store.
    ...
    In fact, there will be at most one fence in push leg for each mover compared to many location updates
    in pull leg.
    ...
    The row in these tables with no values corresponds to a microservice which does not relate to the experiments 19 to 23.

    The following figures illustrate how input rate and throughput have changed during experiments 19 to 23.
    It also shows how many instances of related microservices were up and running during these experiment.
    ...
    input rate (number of location updates per second), yellow line represents throughput (number of fence point
    intersections) and the pink dots shows number of deployed instances of realtime-fencing.
    As we progressed through experiment 19 to 23, we have increased the input rate from around 2k/s to 15k/s.
    At the same time in each experiment we have increased number of deployed instances of realtime-fencing;
    Which is the microservice working as push leg of FenceX.
    ...
    In experiment 23, depicted in figure \ref{fig:ex23} our intention was to increase input rate to above 15k/s which
    we did not manage due to limitations on the available resources to bench-marking application.
