...
\begin{document}
    ...
    Figure \ref{fig:infrastructure} does not include server4 since that server only runs an instance of benchmarking
    ...
    deployed into worker nodes as 16670 MHz of CPU cycles and 16 GB of memory.
    ...
    In on demand style, the input rate is the number of HTTP requests (query by fence) per second that system receives.
    For realtime operations, the input rate is the number of location reports arrives at system per second.
    ...
    example).
    We have deployed one more instance of relevant microservice plus changing the input ate pattern.
    The throughput again followed the input pattern.

    \subsucsection{Conclusion of pure throughput evaluation}
    ...
    \subsucsection{Conclusion of poll throughput evaluation}
    ...
    \subsucsection{Conclusion of push leg availability evaluation}
    ...
    Red line shows how many queries FenceX failed to be responded.
    A failure to respond in this incoming load range means the service supposed to answer the query was not
    ...
    \subsucsection{Conclusion of poll leg availability evaluation}
    ...
    \subsucsection{Conclusion of push leg strong scalability evaluation}
    Push leg of FenceX has strong scalability characteristics at least in the range of input rate managed to produce.
    ...
    in poll leg.
    Please note that at each instance in time, there is only one location record for each mover in location-aggregate
    database (not many).
    ...
    \subsucsection{Conclusion of poll leg strong scalability evaluation}
    Poll leg of FenceX has strong scalability characteristics at least in the range of input rate managed to produce.
    So in case of unexpectedly high load, just scaling realtime-fencing out can prove effective.
    ...
    \subsucsection{Conclusion of push leg weak scalability evaluation}
    ...
    \subsection{Poll leg}
    Poll leg weak scalability evalutaion scenario:
    \begin{itemize}
        \item[1-] Send location updates for movers using all the trips in the bench-marking application database.
    ...
    \subsucsection{Conclusion of push leg free fall scalability evaluation}
