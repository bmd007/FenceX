...
\begin{document}
    ...
    \begin{figure}[ht]
        \caption{Example of physical data flow graph}
    ...
    Two computer science engineers came up with a algorithm and now want to make money out of it.
    They don't want to publish any software or library but an online service, a few APIs (HTTP for example) only.
    They are very confident enough in their work to know that once their service is up and running, lot of companies
    ...
    They need to sit and design different microservices, hire developers to help with implementation and maintenance,
    hire an operations team for maintaining container orchestration tools (among others) and so many other activities
    before even they can start coding. Let alone deploying a feature into production
    ...
    But what if they could just focus on the implementation of their algorithm?
    Just like they are implementing a function in the middle of nowhere.
    After all, they are implementing an audio compression algorithm.
    After all it's just some calculation and computation over an input then producing an output.
    So what if they could just implement that function, give the code to a cloud service with promise of no more work
    or implementation being needed.
    ...
    The cloud provider will take the code, build it somehow, and bring enough instances of the result up and running
    ...
    What we just described is the sell speak for function as a service idea.
    Which is provided nowadays by most of cloud providers.
    AWS lambda is the most famous one.
    ......

    \section{Stateful FAAS}

    \section{Stream processing and FAAS}

    \section{FenceX and FAAS}

    \section{Comparison}

    \subsection{1 maghale faas implementation}
